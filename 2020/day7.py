...
for rule in input.split("\n"):
	...
	children = re.findall(r"([0-9]) ([a-z].*? [a-z].*?) (?=bag)", rule)
	rules[parent] = children

def find_path(rules, start_bag, target_bag, path=[]):
	path = path + [start_bag]
	if start_bag == target_bag:
		return path
	for bag in rules[start_bag]:
		if bag[1] not in path:
				new_path = find_path(rules, bag[1], target_bag, path)
		if new_path:
				return new_path
	return None
	
matches = {k:False for k in rules}
for k, _ in rules.items():
	if matches[k] == False :
		matching_bags = find_path(rules, k, my_bag)
		if matching_bags:
				for new_match in matching_bags:
					matches[new_match] = True
					
print(sum(matches.values()) - 1)

print("---------------------------- Part 2\r\n")
def count_bags(rules, bag, total_bags=1):
	if not rules[bag]:
		return total_bags
	bag_sum = []
	for b in rules[bag]:
		bag_sum += [int(b[0]) * count_bags(rules, b[1], total_bags)]
	return sum(bag_sum) + 1
	
print(count_bags(rules, my_bag) - 1)