class Solution:
	...
	Part 1
	"""
	def countSums(self, groups: [str]) -> int:
		return sum([len(list(set(x.replace("\n", "")))) for x in groups.split("\n\n")])

	def countRepeatedSum(self, groups: str) -> int:
		sum = 0
		for x in groups.split("\n\n"):
			parties = x.split("\n")
			nSet = set(parties[0])
			for p in parties:
				nSet = nSet.intersection(set(p))
			sum += len(nSet)

		return sum

answers = """gwlcpbtxmiezd
xwlcpbtzimgdk
tbiwmpcgzdxo
ygzdbpjxncfwimt
...
print("---------------------------- Part 1\r\n")
sol = Solution()
res = sol.countSums(answers)
print(res)

print("---------------------------- Part 2\r\n")
res = sol.countRepeatedSum(answers)
print(res)